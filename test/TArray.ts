...
import * as TArray from "@effect/stm/TArray"
...
describe.concurrent("TArray", () => {
  ...
  it.effect("findFirstIndex - correct index if in array, with offset", () =>
    Effect.gen(function*($) {
      const array = yield* $(makeRepeats(3, 3))
      const result = yield* $(pipe(array, TArray.findFirstIndex(2, 2)))
      assert.deepStrictEqual(result, Option.some(4))
    }))

  it.effect("findFirstIndex - none if absent after offset", () =>
    Effect.gen(function*($) {
      const array = yield* $(makeRepeats(3, 3))
      const result = yield* $(pipe(array, TArray.findFirstIndex(1, 7)))
      assert.deepStrictEqual(result, Option.none)
    }))

  it.effect("findFirstIndex - none for a negative offset", () =>
    Effect.gen(function*($) {
      const array = yield* $(makeRepeats(3, 3))
      const result = yield* $(pipe(array, TArray.findFirstIndex(1, -1)))
      assert.deepStrictEqual(result, Option.none)
    }))

  it.effect("findFirstIndex - none for an offset that is too large", () =>
    Effect.gen(function*($) {
      const array = yield* $(makeRepeats(3, 3))
      const result = yield* $(pipe(array, TArray.findFirstIndex(1, 9)))
      assert.deepStrictEqual(result, Option.none)
    }))
  ...
  it.effect("findFirstIndexWhere - determines the correct index, with offset", () =>
    Effect.gen(function*($) {
      const n = 10
      const array = yield* $(makeStair(n))
      const result = yield* $(pipe(array, TArray.findFirstIndexWhere((n) => n % 2 === 0, 5)))
      assert.deepStrictEqual(result, Option.some(5))
    }))

  it.effect("findFirstIndexWhere - none if absent after offset", () =>
    Effect.gen(function*($) {
      const n = 10
      const array = yield* $(makeStair(n))
      const result = yield* $(pipe(array, TArray.findFirstIndexWhere((n) => n % 7 === 0, 7)))
      assert.deepStrictEqual(result, Option.none)
    }))

  it.effect("findFirstIndexWhere - none for a negative offset", () =>
    Effect.gen(function*($) {
      const n = 10
      const array = yield* $(makeStair(n))
      const result = yield* $(pipe(array, TArray.findFirstIndexWhere(constTrue, -1)))
      assert.deepStrictEqual(result, Option.none)
    }))

  it.effect("findFirstIndexWhere - none for an offset that is too large", () =>
    Effect.gen(function*($) {
      const n = 10
      const array = yield* $(makeStair(n))
      const result = yield* $(pipe(array, TArray.findFirstIndexWhere(constTrue, n + 1)))
      assert.deepStrictEqual(result, Option.none)
    }))
  ...
  it.effect("findFirstIndexWhereSTM - determines the correct index, with offset", () =>
    Effect.gen(function*($) {
      const n = 10
      const array = yield* $(makeStair(n))
      const result = yield* $(pipe(array, TArray.findFirstIndexWhereSTM((n) => STM.succeed(n % 2 === 0), 5)))
      assert.deepStrictEqual(result, Option.some(5))
    }))

  it.effect("findFirstIndexWhereSTM - none if absent after offset", () =>
    Effect.gen(function*($) {
      const n = 10
      const array = yield* $(makeStair(n))
      const result = yield* $(pipe(array, TArray.findFirstIndexWhereSTM((n) => STM.succeed(n % 7 === 0), 7)))
      assert.deepStrictEqual(result, Option.none)
    }))

  it.effect("findFirstIndexWhereSTM - none for a negative offset", () =>
    Effect.gen(function*($) {
      const n = 10
      const array = yield* $(makeStair(n))
      const result = yield* $(pipe(array, TArray.findFirstIndexWhereSTM(() => STM.succeed(true), -1)))
      assert.deepStrictEqual(result, Option.none)
    }))

  it.effect("findFirstIndexWhereSTM - none for an offset that is too large", () =>
    Effect.gen(function*($) {
      const n = 10
      const array = yield* $(makeStair(n))
      const result = yield* $(pipe(array, TArray.findFirstIndexWhereSTM(() => STM.succeed(true), n + 1)))
      assert.deepStrictEqual(result, Option.none)
    }))
  ...
  it.effect("findFirstIndexWhereSTM - succeeds when error excluded by offset", () =>
    Effect.gen(function*($) {
      const n = 10
      const array = yield* $(makeStair(n))
      const result = yield* $(pipe(
        array,
        TArray.findFirstIndexWhereSTM((n) => n === 1 ? STM.fail("boom") : STM.succeed(n % 5 === 0), 2)
      ))
      assert.deepStrictEqual(result, Option.some(4))
    }))
  ...
  it.effect("findLastIndex - correct index if in array, with limit", () =>
    Effect.gen(function*($) {
      const array = yield* $(makeRepeats(3, 3))
      const result = yield* $(pipe(array, TArray.findLastIndex(2, 6)))
      assert.deepStrictEqual(result, Option.some(4))
    }))

  it.effect("findLastIndex - none if absent before limit", () =>
    Effect.gen(function*($) {
      const array = yield* $(makeRepeats(3, 3))
      const result = yield* $(pipe(array, TArray.findLastIndex(3, 1)))
      assert.deepStrictEqual(result, Option.none)
    }))

  it.effect("findLastIndex - none for a negative limit", () =>
    Effect.gen(function*($) {
      const array = yield* $(makeRepeats(3, 3))
      const result = yield* $(pipe(array, TArray.findLastIndex(2, -1)))
      assert.deepStrictEqual(result, Option.none)
    }))

  it.effect("findLastIndex - none for a limit that is too large", () =>
    Effect.gen(function*($) {
      const array = yield* $(makeRepeats(3, 3))
      const result = yield* $(pipe(array, TArray.findLastIndex(2, 9)))
      assert.deepStrictEqual(result, Option.none)
    }))
  ...
  it.effect("get - should fail when the index is out of bounds", () =>
    Effect.gen(function*($) {
      ...
      assert.deepStrictEqual(Exit.unannotate(result), Exit.die(Cause.RuntimeException("Index out of bounds")))
    }))
